...
if typst compile --root "$root" "$root/tests/invalid-postcode.typ" "$out/invalid-postcode.pdf" 2>"$out/invalid-postcode.log"; then
  printf 'Expected invalid postcode compilation to fail.\n' >&2
  exit 1
fi
rg -q 'postcode must contain exactly four digits' "$out/invalid-postcode.log"
